USE P4
SELECT 'R1: A -- > B' AS FD, CASE WHEN COUNT(*)=0 THEN 'GILDIR' ELSE 'gildir ekki' END AS VALIDITY
FROM(
SELECT A
FROM R1
GROUP BY A
...
SELECT 'R1: B -- > A' AS FD, CASE WHEN COUNT(*)=0 THEN 'GILDIR' ELSE 'gildir ekki' END AS VALIDITY
FROM(
SELECT B
FROM R1
GROUP BY B
...
SELECT 'R1: A -- > C' AS FD, CASE WHEN COUNT(*)=0 THEN 'GILDIR' ELSE 'gildir ekki' END AS VALIDITY
FROM(
SELECT A
FROM R1
GROUP BY A
...
SELECT 'R1: C -- > A' AS FD, CASE WHEN COUNT(*)=0 THEN 'GILDIR' ELSE 'gildir ekki' END AS VALIDITY
FROM(
SELECT C
FROM R1
GROUP BY C
